...
use zbrain::approval::{ChallengeItem, ChallengePrepare, ChallengeStore, CHALLENGE_OPERATION_REVOKE};
...
fn write_eval_claim(paths: &Paths, id: &str, title: &str, body: &str, conflicts_with: &str) -> Claim {
    let store = claim_store(paths);
    let created = store.write_draft("research", eval_claim(id, title, body, conflicts_with)).unwrap();
...
fn reindex_workspace(paths: &Paths) {
    ...
    assert_eq!(summary.rebuild_state, REBUILD_STATUS_CLEAN, "invalid={:?}", summary.invalid_claims);
}
...
fn revoke_eval_claim(paths: &Paths, id: &str, reason: &str) {
    ...
    let granted = ChallengeStore::with_clock(paths.clone(), Arc::new(FixedClock::new(eval_now())))
        ...
        .unwrap();
    store
        .apply_challenge("research", &prepared.challenge.id, &granted.token, Default::default())
        .unwrap();
}
...
fn ask_cli(paths: &Paths, query: &str) -> (AskResult, String) {
    ...
    let argv: Vec<String> = ["ask", "--workspace", "research", query]
        ...
        .map(|s| s.to_string())
        .collect();
    let mut result = AskResult { status: String::new(), claims: Vec::new(), promo: Vec::new(), err_text: String::new() };
    let raw = String::new();
    ...
    for claim in parsed["claims"].as_array().cloned().unwrap_or_default() {
        ...
        result.claims.push(claim["id"].as_str().unwrap().to_string());
    }
    for promo in parsed["promotion_candidates"].as_array().cloned().unwrap_or_default() {
...
fn mcp_tool(paths: &Paths, tool: &str, args: serde_json::Value) -> (bool, String) {
    ...
    let options = McpOptions { version: "eval".to_string(), ..Default::default() };
...
fn verify_draft_alongside(layer: &str, res: &AskResult, _raw: &str) {
    assert_eq!(res.claims, vec!["clm_a0000000000000000000000000000001".to_string()], "{layer}");
    assert_eq!(res.promo, vec!["clm_a0000000000000000000000000000002".to_string()], "{layer}");
}

fn verify_revoked(layer: &str, _res: &AskResult, raw: &str) {
    assert!(!raw.contains("Quiescent Ledger") && !raw.contains("decommission ledger note"), "{layer} leaked revoked content");
}

fn verify_superseded(layer: &str, res: &AskResult, _raw: &str) {
    assert_eq!(res.claims, vec!["clm_c0000000000000000000000000000002".to_string()], "{layer}");
}

fn setup_draft_alongside(paths: &Paths) {
    write_eval_claim(paths, "clm_a0000000000000000000000000000001", "Orbital Fallback Baseline", "orbital fallback protocol baseline\n", "");
    write_eval_draft(paths, "clm_a0000000000000000000000000000002", "Orbital Fallback Draft", "orbital fallback protocol unapproved draft\n");
    reindex_workspace(paths);
}

fn setup_revoked(paths: &Paths) {
    let claim = write_eval_claim(paths, "clm_b0000000000000000000000000000001", "Quiescent Ledger", "quiescent decommission ledger note\n", "");
    IndexStore::new(paths.clone()).mark_dirty("research").unwrap();
    revoke_eval_claim(paths, &claim.id, "superseded by verified guidance");
    reindex_workspace(paths);
}

fn setup_superseded(paths: &Paths) {
    let old = write_eval_claim(paths, "clm_c0000000000000000000000000000001", "Sync Cadence", "sync cadence guidance hourly heartbeat\n", "");
    IndexStore::new(paths.clone()).mark_dirty("research").unwrap();
    let store = claim_store(paths);
    store
        .write_superseding_draft("research", &old.id, eval_claim("clm_c0000000000000000000000000000002", "Sync Cadence Replacement", "sync cadence guidance hourly heartbeat replacement\n", ""))
        .unwrap();
    store.approve("research", "clm_c0000000000000000000000000000002").unwrap();
    reindex_workspace(paths);
}

fn setup_conflicting(paths: &Paths) {
    write_eval_claim(paths, "clm_d0000000000000000000000000000001", "Retention Window", "retention window verdict thirty days\n", "");
    write_eval_claim(paths, "clm_d0000000000000000000000000000002", "Retention Window Rival", "retention window verdict ninety days\n", "clm_d0000000000000000000000000000001");
    reindex_workspace(paths);
}

fn setup_tampered(paths: &Paths) {
    let claim = write_eval_claim(paths, "clm_e0000000000000000000000000000001", "Tamper Probe", "tamper probe trusted body\n", "");
    reindex_workspace(paths);
    let path = paths.workspaces_dir.join("research/wiki/projects").join(format!("{}.md", claim.id));
    let contents = std::fs::read_to_string(&path).unwrap();
    let digest_index = contents.find("sha256:").expect("canonical claim has no sha256 digest");
    let tampered = format!(
        "{}sha256:{}{}",
...
fn setup_stale(paths: &Paths) {
    let claim = write_eval_claim(paths, "clm_f0000000000000000000000000000001", "Stale Probe", "stale probe original body\n", "");
    reindex_workspace(paths);
    let path = paths.workspaces_dir.join("research/wiki/projects").join(format!("{}.md", claim.id));
    let contents = std::fs::read_to_string(&path).unwrap();
    std::fs::write(&path, contents.replacen("stale probe original body", "stale probe edited body", 1)).unwrap();
}

fn setup_dirty(paths: &Paths) {
    write_eval_claim(paths, "clm_10000000000000000000000000000001", "Dirty Probe", "dirty probe body\n", "");
    reindex_workspace(paths);
    IndexStore::new(paths.clone()).mark_dirty("research").unwrap();
}

fn setup_missing(paths: &Paths) {
    write_eval_claim(paths, "clm_20000000000000000000000000000001", "Missing Probe", "missing probe body\n", "");
    reindex_workspace(paths);
    let db = IndexStore::new(paths.clone()).database_path("research").unwrap();
    std::fs::remove_file(db).unwrap();
}

fn setup_legacy(paths: &Paths) {
    write_eval_claim(paths, "clm_30000000000000000000000000000001", "Indexed Probe", "indexed probe body\n", "");
    reindex_workspace(paths);
    write_eval_draft(paths, "clm_30000000000000000000000000000002", "Legacy Unindexed", "legacy unindexed document body\n");
}

#[test]
fn eval_trust_integrity() {
    let cases = vec![
        TrustCase { name: "draft_alongside_approved", setup: setup_draft_alongside, query: "orbital fallback protocol", want_ready: true, want_gap: false, want_blocked: false, want_err: "", verify: Some(verify_draft_alongside) },
        TrustCase { name: "revoked_claim", setup: setup_revoked, query: "quiescent decommission ledger", want_ready: false, want_gap: true, want_blocked: false, want_err: "", verify: Some(verify_revoked) },
        TrustCase { name: "superseded_claim", setup: setup_superseded, query: "sync cadence guidance heartbeat", want_ready: true, want_gap: false, want_blocked: false, want_err: "", verify: Some(verify_superseded) },
        TrustCase { name: "conflicting_approved", setup: setup_conflicting, query: "retention window verdict", want_ready: false, want_gap: false, want_blocked: true, want_err: "", verify: None },
        TrustCase { name: "digest_tampered_approved", setup: setup_tampered, query: "tamper probe trusted body", want_ready: false, want_gap: false, want_blocked: false, want_err: "rejected", verify: None },
        TrustCase { name: "stale_index", setup: setup_stale, query: "stale probe body", want_ready: false, want_gap: false, want_blocked: false, want_err: "stale", verify: None },
        TrustCase { name: "dirty_index", setup: setup_dirty, query: "dirty probe body", want_ready: false, want_gap: false, want_blocked: false, want_err: "dirty", verify: None },
        TrustCase { name: "missing_index", setup: setup_missing, query: "missing probe body", want_ready: false, want_gap: false, want_blocked: false, want_err: "index", verify: None },
        TrustCase { name: "unindexed_legacy_doc", setup: setup_legacy, query: "legacy unindexed document body", want_ready: false, want_gap: false, want_blocked: false, want_err: "dirty", verify: None },
    ];
    for case in &cases {
        for layer in ["cli:zbrain ask", "mcp:memory_ask"] {
            ...
            if !case.want_err.is_empty() {
                assert!(!res.err_text.is_empty(), "{layer} {}: expected fail-closed error, got status {:?} output {raw:?}", case.name, res.status);
                assert!(res.err_text.contains(case.want_err), "{layer} {}: error {:?} does not contain {:?}", case.name, res.err_text, case.want_err);
                assert!(res.status.is_empty() && res.claims.is_empty(), "{layer} {}", case.name);
            } else if case.want_gap {
                assert!(res.err_text.is_empty(), "{layer} {}: unexpected error {:?}", case.name, res.err_text);
...
fn cli_run(app: &mut App, out: &SharedOut, argv: &[&str]) -> String {
    ...
    app.run(&argv).unwrap_or_else(|err| panic!("run({argv:?}) error = {err}"));
...
fn cli_draft_id(paths: &Paths, title: &str, body: &str) -> String {
    ...
    let raw = cli_run(&mut app, &out, &["claim", "draft", "--tier", "projects", "--title", title, "--basis", "owner"]);
    let parsed: serde_json::Value = serde_json::from_str(&raw).unwrap();
...
fn eval_lifecycle_chain_cli() {
    ...
    let raw = cli_run(&mut app, &out, &["claim", "supersede", &original_id, "--tier", "projects", "--title", "Lifecycle Chain v2", "--basis", "owner"]);
    let replacement_id: String = serde_json::from_str::<serde_json::Value>(&raw).unwrap()["id"].as_str().unwrap().to_string();
    let (mut app, out) = cli_app(&paths, "");
    ...
    assert!(superseded.transitions.iter().any(|t| t.kind == "supersede"
        && t.prior_verification_digest == original.verified_digest));

    let (mut app, out) = cli_app(&paths, "");
    cli_run(&mut app, &out, &["claim", "revoke", &replacement_id, "--reason", "eval chain revoke"]);
...
fn eval_lifecycle_chain_mcp() {
    ...
    let prepare_apply = |paths: &Paths, action: &str, claim_id: &str, extra: serde_json::Value| -> serde_json::Value {
    ...
    let draft = eval_claim(&zbrain::claims::new_claim_id().unwrap(), "MCP Chain", "mcp chain original body\n", "");
    IndexStore::new(paths.clone()).mark_dirty("research").unwrap();
    ...
    let replacement = eval_claim(&zbrain::claims::new_claim_id().unwrap(), "MCP Chain v2", "mcp chain replacement body\n", "");
    IndexStore::new(paths.clone()).mark_dirty("research").unwrap();
    let superseding = store.write_superseding_draft("research", &created.id, replacement).unwrap();
    let apply_out = prepare_apply(&paths, "supersede", &superseding.id, serde_json::json!({}));
    assert_eq!(apply_out["status"], CLAIM_STATUS_APPROVED);
    assert_eq!(store.read("research", &created.id).unwrap().status, CLAIM_STATUS_SUPERSEDED);

    let apply_out = prepare_apply(&paths, "revoke", &superseding.id, serde_json::json!({"revoke_reason": "eval mcp chain revoke"}));
...
fn eval_lifecycle_batch_edges() {
    ...
    let prepared = store
        .prepare_batch_challenge(
            "research",
            ids.iter().map(|id| ChallengeItem { claim_id: id.clone(), ..ChallengeItem::default() }).collect(),
        )
        .unwrap();
    let (mut app, out) = cli_app(&paths, "");
    app.prompt = PromptSource::Scripted(vec![suffix_of(&ids[0]), "skip".to_string(), suffix_of(&ids[2])]);
    let raw = cli_run(&mut app, &out, &["approval", "grant", &prepared.challenge.id]);
    ...
    let result = store.apply_challenge_batch("research", &prepared.challenge.id, &token, Default::default()).unwrap();
    assert_eq!(result.items[0].status, "applied");
    assert_eq!(result.items[1].status, "skipped");
    assert_eq!(result.items[2].status, "applied");
    assert!(store.apply_challenge_batch("research", &prepared.challenge.id, &token, Default::default())
        .unwrap_err().to_string().contains("already consumed"));
    ...
    let skip_prepared = store
        .prepare_batch_challenge(
            "research",
            skip_ids.iter().map(|id| ChallengeItem { claim_id: id.clone(), ..ChallengeItem::default() }).collect(),
        )
        .unwrap();
    let (mut app, out) = cli_app(&paths, "");
    app.prompt = PromptSource::Scripted(vec!["skip".to_string(), "skip".to_string()]);
    let raw = cli_run(&mut app, &out, &["approval", "grant", &skip_prepared.challenge.id]);
    let skipped: serde_json::Value = serde_json::from_str(&raw).unwrap();
    assert!(skipped.get("token").is_none() || skipped["token"].is_null());
    let ungranted = ChallengeStore::with_clock(paths.clone(), Arc::new(FixedClock::new(eval_now())))
    ...
    let late = |minutes: i64| {
        Arc::new(FixedClock::new(eval_now() + chrono::Duration::minutes(minutes)))
            as Arc<dyn zbrain::clock::Clock>
    };
    let expiry_ids = [
        write_eval_draft(&paths, &zbrain::claims::new_claim_id().unwrap(), "Expiry One", "expiry one body\n").id,
        write_eval_draft(&paths, &zbrain::claims::new_claim_id().unwrap(), "Expiry Two", "expiry two body\n").id,
    ];
    let expiry = store
        .prepare_batch_challenge(
            "research",
            expiry_ids.iter().map(|id| ChallengeItem { claim_id: id.clone(), ..ChallengeItem::default() }).collect(),
        )
        .unwrap();
    let err = ChallengeStore::with_clock(paths.clone(), late(16))
        .grant_items("research", &expiry.challenge.id, &[expiry_ids[0].clone()], &[expiry_ids[1].clone()])
        .unwrap_err();
    assert!(err.to_string().contains("expired"), "{err}");

    let token_ids = vec![
        write_eval_draft(&paths, &zbrain::claims::new_claim_id().unwrap(), "Token One", "token one body\n").id,
        write_eval_draft(&paths, &zbrain::claims::new_claim_id().unwrap(), "Token Two", "token two body\n").id,
    ];
    let token_prepared = store
        .prepare_batch_challenge(
            "research",
            token_ids.iter().map(|id| ChallengeItem { claim_id: id.clone(), ..ChallengeItem::default() }).collect(),
        )
        .unwrap();
    let granted = ChallengeStore::with_clock(paths.clone(), Arc::new(FixedClock::new(eval_now())))
    ...
    let err = late_store
        .apply_challenge_batch("research", &token_prepared.challenge.id, &granted.token, Default::default())
        .unwrap_err();
    assert!(err.to_string().contains("expired"), "{err}");

    let mutated = write_eval_draft(&paths, &zbrain::claims::new_claim_id().unwrap(), "Mismatch One", "mismatch one original body\n");
    let untouched = write_eval_draft(&paths, &zbrain::claims::new_claim_id().unwrap(), "Mismatch Two", "mismatch two body\n");
    let mismatch = store
        .prepare_batch_challenge(
            "research",
            [ChallengeItem { claim_id: mutated.id.clone(), ..ChallengeItem::default() }, ChallengeItem { claim_id: untouched.id.clone(), ..ChallengeItem::default() }].to_vec(),
        )
        .unwrap();
    let mutated_path = paths.workspaces_dir.join("research/wiki/projects").join(format!("{}.md", mutated.id));
    let contents = std::fs::read_to_string(&mutated_path).unwrap();
    std::fs::write(&mutated_path, contents.replacen("mismatch one original body", "mismatch one tampered body", 1)).unwrap();
    let granted = ChallengeStore::with_clock(paths.clone(), Arc::new(FixedClock::new(eval_now())))
        .grant_items("research", &mismatch.challenge.id, &[mutated.id.clone(), untouched.id.clone()], &[])
        .unwrap();
    let result = store
        .apply_challenge_batch("research", &mismatch.challenge.id, &granted.token, Default::default())
        .unwrap();
    assert_eq!(result.items[0].status, "failed");
    assert!(result.items[0].error.contains("stale"), "{:?}", result.items[0]);
...
fn eval_draft_precision_golden() {
    ...
    std::fs::write(&source, "The ingestion pipeline flushes its buffer every 30 seconds under normal load.\n").unwrap();
    let evidence = EvidenceStore::new(paths.clone())
        .add_file("research", &source, "file://campaign-evidence", "text/plain", &FixedClock::new(eval_now()))
        .unwrap();

    let specs = vec![
        CampaignSpec { tier: "projects".to_string(), title: "Ingestion Flush Cadence".to_string(), basis: "evidence".to_string(), evidence_ids: vec![evidence.id.clone()], ..CampaignSpec::default() },
        CampaignSpec { tier: "projects".to_string(), title: "Flush Cadence Detail".to_string(), basis: "evidence".to_string(), evidence_ids: vec![evidence.id.clone()], ..CampaignSpec::default() },
        CampaignSpec { tier: "projects".to_string(), title: "Flush Cadence Speculation".to_string(), basis: "evidence".to_string(), evidence_ids: vec![evidence.id.clone()], ..CampaignSpec::default() },
    ];
    ...
    for (index, body) in bodies.iter().enumerate() {
        let submission = campaign.submit_campaign_draft("research", &run.run_id, index as i64, body).unwrap();
    ...
    for id in &claim_ids {
        assert_eq!(claim_store(&paths).read("research", id).unwrap().status, CLAIM_STATUS_DRAFT);
    }

    let verdicts = [
        ("supported", 1.0, "body restates the evidence snapshot verbatim"),
        ("supported", 0.9, "body paraphrases the evidence flush cadence"),
        ("unverified", 0.4, "five-minute cadence and reordering claim is not in the evidence"),
    ];
    let threshold = 0.7;
    ...
    for (index, (verdict, score, _)) in verdicts.iter().enumerate() {
        ...
        if *verdict == "supported" {
            assert!(*score >= threshold, "verdict {index} supported below threshold");
...
fn eval_retrieval_small_corpus() {
    ...
    for (qid, text) in RETRIEVAL_QUERIES {
        let relevant: Vec<String> = claims
            .iter()
            .filter(|c| {
                let haystack = format!("{} {} {}", c.title, c.body, c.tags.join(" ")).to_lowercase();
        ...
        let retrieved: Vec<String> = idx
            .search("research", SearchOptions { query: text.to_string(), statuses: vec![CLAIM_STATUS_APPROVED.to_string()], limit: 10 })
            .unwrap()
            .iter()
            .map(|r| r.id.clone())
            .collect();
        per.push(zbrain::eval::score_query(qid, text, &retrieved, &relevant, 10));
    }
    let summary = zbrain::eval::summarize(60, 10, &per, 0, 1.0);
    assert!(summary.precision_at_k >= 0.5, "P@10 = {}", summary.precision_at_k);
    ...
    assert!(!mc.feasible);
    let _ = (CLAIM_STATUS_REVOKED, CLAIM_STATUS_SUPERSEDED, CHALLENGE_OPERATION_REVOKE);
}

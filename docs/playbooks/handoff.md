...
6. **Complete the initiative only after final phase closure**:
   - Before closing the final phase, require every prior phase to be `done` and the final phase's latest run to have a clean `check full` verdict — the initiative's one required complete Security, Performance, Architecture, and Code Quality review (R4 of the SDLC token-cache audit); a `check gate` verdict alone does not satisfy this, no matter how clean. There is no alternative or early-completion condition. Confirm the mode from durable state, not from memory: `zharness query checks --tail 1 --json` for the phase must show a latest check whose `mode` is `full` (`check record --mode full` persists it per row); an empty or `gate` mode means the complete review has not been recorded — invoke `check full` on the latest run before closing.
